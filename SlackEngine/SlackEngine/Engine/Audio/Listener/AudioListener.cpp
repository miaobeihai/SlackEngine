...
void SlackEngine::Audio::AudioListener::set_listener_location(const Core::SVector3& location)
{
	alListener3f(AL_POSITION, location.get_x(), location.get_y(), location.get_z());
}

void SlackEngine::Audio::AudioListener::set_listener_velocity(const Core::SVector3& velocity)
{
	alListener3f(AL_VELOCITY, velocity.get_x(), velocity.get_y(), velocity.get_z());
}

void SlackEngine::Audio::AudioListener::set_listener_orientation(
