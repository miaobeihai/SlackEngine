...
MainMenu::MainMenu(SlackEngine::Core::LogicManagerView* logic_view, Ball* player,
...
void MainMenu::on_gui()
{
	if (!game_started_)
	{
		...
		{
			ImGui::Text("SlackEngine - Example game");
...
{
	input_ref_->load_system_cursor(SlackEngine::Input::system_cursor_shapes::cursor_hand_shape);
}

void MainMenu::set_arrow_cursor() const
{
	input_ref_->load_system_cursor(SlackEngine::Input::system_cursor_shapes::cursor_regular_arrow);
}
