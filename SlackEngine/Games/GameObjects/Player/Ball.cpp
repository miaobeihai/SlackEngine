...
Ball::Ball(SlackEngine::Core::ComponentsManager* input_component_manager,
	  ...
	  input_manager_ref_(created_input_managerf)
{
	...
	set_object_scale(SlackEngine::Core::SVector3(0.5f, 0.5f, 0.5f));
	set_spawnpoint(SlackEngine::Core::SVector3(0, -10, 0));
	...
	collider_ = input_component_manager->create_sphere_rigidbody_component(
		10.f,
		SlackEngine::Core::SVector3(0, -10, 0), 8.f);
...
void Ball::game_update(const float delta_time)
{
	...
	if (can_move_)
	{
		...
		{
			collider_->apply_force_to_center(SlackEngine::Core::SVector3(0, 0, -force_to_apply * (delta_time * 100)));
		}
		...
		{
			collider_->apply_force_to_center(SlackEngine::Core::SVector3(-force_to_apply * (delta_time * 100), 0, 0));
		}
...
void Ball::set_can_move(const bool can_move)
{
	...
	if (can_move)
	{
		input_manager_ref_->set_cursor_input_mode(SlackEngine::Input::cursor_mode::cursor_grabbed_mode);
	}
	else
	{
		input_manager_ref_->set_cursor_input_mode(SlackEngine::Input::cursor_mode::cursor_normal_mode);
	}
}
...
void Ball::set_spawnpoint(const SlackEngine::Core::SVector3& pos)
{
	respawn_point_ = pos;
}

SlackEngine::Core::SVector3 Ball::get_rigidbody_location() const
{
...
{
	const SlackEngine::Core::SVector3 start_pos = get_rigidbody_location();
	const SlackEngine::Core::SVector3 end_pos = start_pos - SlackEngine::Core::SVector3(0, 50, 0);
	const SlackEngine::Core::raycast_result result = component_manager_ref_->execute_single_raycast(start_pos, end_pos);
