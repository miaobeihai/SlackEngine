...
GameCamera::GameCamera(SlackEngine::Input::InputManager* created_input_managerf,
...
void GameCamera::set_game_window_ref(SlackEngine::Render::GameWindow* window_ref)
{
...
{
	set_object_location(SlackEngine::Core::SVector3(0.20f, 150.f, 234.f));
	set_object_rotation(SlackEngine::Core::SVector3(-40.f, -90, 0));
}
...
void GameCamera::game_update(const float time)
{
	...
	if (input_manager_ref_->get_keyboard_key_pressed(KEYBOARD_KEY_F) && !free_camera_key_pressed_)
	{
		if (player_ref_->get_can_move())
		{
			...
			{
				camera_component_->set_component_rotation(SlackEngine::Core::SVector3(-40.f, -90, 0));
			}
		}
	}
	...
	if (!free_camera_)
	{
		SlackEngine::Core::SVector3 player_location = player_ref_->get_rigidbody_location();
	...
	{
		//look position
		const SlackEngine::Input::mouse_location mouse = input_manager_ref_->get_last_mouse_location();
		camera_component_->process_mouse_movement(static_cast<float>(mouse.xpos), static_cast<float>(mouse.ypos), true);

		//speed
		const SlackEngine::Input::scroll_status scroll = input_manager_ref_->get_mouse_scroll_status();
		if (scroll == SlackEngine::Input::scroll_status::scroll_up)
		{
			camera_multiplier_++;
		}
		else if (scroll == SlackEngine::Input::scroll_status::scroll_down)
		{
			camera_multiplier_--;
		}
		...
		if (input_manager_ref_->get_keyboard_key_pressed(KEYBOARD_KEY_D))
		{
			set_object_location(get_object_location() +
				(
					SlackEngine::Core::SVector3::cross_product(camera_component_->get_camera_front(),
	...
	{
		SlackEngine::Debug::DebugLog::print_to_console_log("ESC pressed - leaving game");
