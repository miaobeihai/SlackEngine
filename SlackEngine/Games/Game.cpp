...
int main()
{
	...
	SlackEngine::Manager::EngineManager* scrap_engine_manager = nullptr;
	try
	{
		//Init engine
		scrap_engine_manager = new SlackEngine::Manager::EngineManager("Example Game", 0, 1280, 720);
		SlackEngine::Render::GameWindow* game_window_ref = scrap_engine_manager->render_manager_view->get_game_window();
		...
		SlackEngine::Core::ComponentsManager* component_manager_ref = scrap_engine_manager
		                                                              ->logic_manager_view->get_components_manager();
		//Set the world skybox
		SlackEngine::Core::SceneManager* scene_manager_ref = scrap_engine_manager
